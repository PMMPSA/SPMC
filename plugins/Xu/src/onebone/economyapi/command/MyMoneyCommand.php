...
class MyMoneyCommand extends Command{
	...
	public function __construct(EconomyAPI $plugin){
		parent::__construct("mymoney", "xem số xu hiện có", "/mymoney", ["mymoney", "xu"]);
...
/*		if(!$sender->hasPermission($this->getPermission())){
		...
		$sender->sendMessage("§lSố xu hiện có: §e" . $this->plugin->myMoney($sender));
		//$sender->sendMessage("순위 : 전체 " . number_format($total) . "명중 " . number_format($this->plugin->getRank($sender)) . "위");
		return true;
	}
}
