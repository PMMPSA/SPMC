...
use Itzdvbravo\BravoClan\Main as Clan;
class PureChat extends PluginBase
{
    ...
    */
    ...
    public function applyPCTags($string, Player $player, $message, $levelName)
    {
        ...
        $string = str_replace("{clan}", $this->getClan($player), $string);
        return $string;
    }


    public function getClan($player){
        if (Clan::$file->isInClan(strtolower($player->getname()))) {
            $clan = Clan::$file->getClan(Clan::$clan->player[strtolower($player->getName())]);
            return "§l§f〖§r".$clan["clan"]."§f〗§r";
        }
        return "";
    }
