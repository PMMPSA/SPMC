...
class Loader extends PluginBase implements Listener {
        ...
        if($tile instanceof CrateTile){
            ...
            if($item->equals($tile->getCrate()->getKey())){
                if($tile->isInUse() == false){
                    ...
                    $player->sendMessage(TextFormat::GRAY . "§l§6BOX  §aCó người khác đang sử dụng, vui lòng chờ!");
                }
            }else{
                if($tile->getCrate()->getMoneyCost() > -1 or $tile->getCrate()->getXpCost() > -1){
                    $player->sendForm(new ConfirmOpenForm($tile));
                }else{
                    $player->sendMessage(TextFormat::GRAY . "§l§6BOX  §aBạn phải cầm key trên tay, nếu không có sử dụng lệnh§d /lcoin§a để mua");
                }
            }
        }
    }
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool {
        ...
        if($sender instanceof Player){
            if($sender->getName() != "phuongaz") return false;
        }
        switch($sub){
            case "create":
                if($sender instanceof Player){
            ...
            case "key":
                ...
                $player->sendMessage(TextFormat::GREEN . "§l§6BOX  §bMua thành công");
            break;
            default:
